...
resource "azurerm_subnet" "gateway_subnet" {
  name                 = lower(format("GatewaySubnet-%s", var.env))
  resource_group_name  = var.resource_group
...
resource "azurerm_local_network_gateway" "on_prem" {
  name                = lower(format("%s-on-premises-lgw", replace(var.location_name, " ", "-")))
  location            = var.location
  resource_group_name = var.resource_group
...
resource "azurerm_virtual_network_gateway_connection" "vpn_connection" {
  name                = lower(format("%s-vpn-to-onprem", replace(var.location_name, " ", "-")))
  location            = var.location
  resource_group_name = var.resource_group
